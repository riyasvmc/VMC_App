...
public class DialogFragment_AddExpiryDate extends DialogFragment {
    ...
    public void add() {
        ...
        if(edit_mode){
            reference = ((DatabaseReference)Data.getQuery(getActivity(), Data.EXPIRY_DATES)).child(item.getKey());
            item_new = new ExpiryDate(item.getKey(), title, period,item.getCreatedAt());
        }else {
            reference = ((DatabaseReference)Data.getQuery(getActivity(), Data.EXPIRY_DATES)).push();
