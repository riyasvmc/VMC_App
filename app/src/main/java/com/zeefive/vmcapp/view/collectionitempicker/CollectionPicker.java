package com.zeefive.vmcapp.view.collectionitempicker;
...
public class CollectionPicker extends LinearLayout {
    ...
    private int mLayoutBackgroundColorNormal = Color.BLUE;
    private int mLayoutBackgroundColorPressed = Color.RED;
    ...
    public void drawItemView() {
        ...
        for (int i = 0; i < mItems.size(); i++) {
            ...
            float itemWidth = itemTextView.getPaint().measureText(item.text) + textPaddingLeft + textPaddingRight;
            ...
            itemWidth += Utils.dpToPx(getContext(), 30) + textPaddingLeft + textPaddingRight;

            if (mWidth <= totalPadding + itemWidth + Utils.dpToPx(this.getContext(), LAYOUT_WIDTH_OFFSET)) {
